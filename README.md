...
依次训练 5 个压缩比：`[0.01, 0.04, 0.10, 0.25, 0.50]`，每轮 300 epoch。
...
| `--wd` | 0.05 | AdamW 权重衰减 |
| `--grad_clip` | 1.0 | 梯度裁剪 |
| `--nll_warmup` | 30 | NLL 预热轮数 (前 30 epoch 纯 MSE) |
| `--beta_nll` | 0.5 | β-NLL 参数 |

## GPU 需求

8 轮深度展开 + 3D DST 注意力在 fp32 下显存占用较高（~5.57M 参数，但中间激活很大）：

| GPU | 显存 | batch=32 | 推荐 |
|-----|------|----------|------|
| **A100** | 80 GB | 轻松跑，可开启 AMP(fp16) | **最佳选择** |
| A6000 / RTX 6000 Ada | 48 GB | 可跑，fp32 无压力 | 次选 |
| RTX 4090 / 3090 | 24 GB | 需降到 batch=8 | 预算选项 |
| RTX 5080 (Blackwell) | 16 GB | 不可行，需 batch≤4 + 自动禁用 AMP | 仅测试用 |

AMP 在非 Blackwell 卡上自动启用（A100/A6000/4090），RTX 5080 自动回退 fp32。
